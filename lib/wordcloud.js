...
// Function to check if dark mode is active (works with manual toggle)
function isCurrentlyDarkMode() {
  // Check for manual theme class first
...
function getColorPalette() {
  return isCurrentlyDarkMode() ? dark_color_pallette1 : light_color_pallette;
}

// Store the fill function for color updates
var currentFill = d3.scale.ordinal().range(getColorPalette());
...
// Listen for theme change events - just update colors, don't re-render
window.addEventListener('themechange', function() {
  setTimeout(function() {
    var newFill = d3.scale.ordinal().range(getColorPalette());
    d3.select("#wordcloud").selectAll("text")
      .transition()
      .duration(300)
      .style("fill", function(d) {
        return newFill(d.text.toLowerCase());
      });
  }, 50);
});
