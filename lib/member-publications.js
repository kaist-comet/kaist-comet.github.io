...
(function() {
    // Highlight the author's name in a publication element
    function highlightAuthorName(element, authorName) {
        // Walk through all text nodes and replace the author name with a highlighted version
        ...
        textNodes.forEach(function(textNode) {
            if (textNode.nodeValue.indexOf(authorName) !== -1) {
                var parts = textNode.nodeValue.split(authorName);
                var fragment = document.createDocumentFragment();
                parts.forEach(function(part, index) {
                    if (index > 0) {
                        var highlight = document.createElement('span');
                        highlight.className = 'author-highlight';
                        highlight.textContent = authorName;
            ...
            containers.forEach(function(container, containerIndex) {
                ...
                allPublications.forEach(function(pub) {
                    ...
                    if (pubText.indexOf(authorName) !== -1) {
                        ...
                        // Highlight the author's name in the publication
                        highlightAuthorName(clone, authorName);
                ...
                if (matchingPubs.length > 0) {
                    var card = container.closest('.member-card');
                    if (!card) return;
                    var links = card.querySelector('.member-card__links');
                    if (!links) {
                        links = document.createElement('div');
                        links.className = 'member-card__links';
                        card.querySelector('.member-card__main').appendChild(links);
                    }

                    var panelId = 'member-papers-' + containerIndex;
                    container.id = panelId;
                    container.hidden = true;
                    container.setAttribute('role', 'region');
                    container.setAttribute('aria-label', authorName + ': Papers');

                    var button = document.createElement('button');
                    button.type = 'button';
                    button.className = 'member-card__link papers-toggle';
                    button.title = 'Papers';
                    button.setAttribute('aria-label', authorName + ': Papers');
                    button.setAttribute('aria-expanded', 'false');
                    button.setAttribute('aria-controls', panelId);
                    var icon = document.createElement('img');
                    icon.src = '/images/papers.svg';
                    icon.alt = '';
